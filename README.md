...
- **`routes`** Page patterns markdown is negotiated for, as strings or `{ path, raw }` objects. `*` matches one path segment, `**` matches one or more, so a locale prefix or an entire nested tree is one pattern and the generated CDN route table stays O(patterns), never O(pages). `raw` overrides the raw destination and is only honored on exact (non-wildcard) patterns, e.g. `{ path: '/', raw: '/raw/index.md' }`, and must sit under `rawPrefix`; wildcard patterns always resolve to `rawPrefix + path + '.md'`. The patterns decide what negotiates, not what the source can serve: under the default `['/', '/**']` a hand-written Vue page outside the content directory (a `/pricing` page on a docs-only site) answers agents with a markdown 404 while browsers keep the HTML, deliberately, so an agent can tell an unknown URL from an empty one. On a site mixing hand-written pages with a partial content directory, narrow the patterns to the documented tree (`['/', '/docs/**']`) or list those pages in `excludePrefixes`.
